...
std::unique_ptr<unsigned char[]> Simulate::render() {
...
void Simulate::prepare_renderloop() {
  ...
  this->platform_ui->ToggleFullscreen();
}

void Simulate::renderloop(int max_timesteps, std::string video_fname) {
  // run event loop
  int t = 0;

  // define the codec and create a VideoWriter object
  int codec = cv::VideoWriter::fourcc('M', 'J', 'P', 'G');
  cv::Size frame_size(1400, 900);
  int frame_rate = 30;
  cv::VideoWriter writer("videos/" + video_fname + ".avi", codec, frame_rate, frame_size, true);
  ...
  cv::Mat frame_bgr(frame_size, CV_8UC3);

  while (t < max_timesteps && !this->platform_ui->ShouldCloseWindow()  && !this->exitrequest.load()) {
    {
    ...
    rgb = this->render();
    std::memcpy(frame.data, rgb.get(), 1400 * 900 * 3 * sizeof(unsigned char));
